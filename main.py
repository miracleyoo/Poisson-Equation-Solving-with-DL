...
folder_init(opt)
gen_name(opt)
train_pairs, test_pairs, class_names = dou_load_data()#load_data("./Datasets/"+opt.DATASET_PATH + '/')
opt.NUM_CLASSES = len(class_names)

trainDataset = DouDat(train_pairs, opt, transform_train)
...
testDataset  = DouDat(test_pairs, opt, transform_test)
...
net = dou_resnet.train_dou_resnet(net, opt)

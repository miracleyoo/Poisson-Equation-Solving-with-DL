...
# Gm = matlabarray([331279549456609,354194616539726,247052833482116,240030204722407])
Gm_max = max(Gmh, Gml)
...
for a in arange(1, length(Vp_all)).reshape(-1):
    for b in arange(1, length(Vn_all)).reshape(-1):
        ...
        for ti in arange(1, tsteps).reshape(-1):
            ...
            index_v1 = (valley[:, 1] == 1)
            index_v1 = index_v1[:, 1]*arange(1, max_particles)
            index_v1 = index_v1 * (index_v1 != 0)
            index_v2 = (valley[:, 1] == 2)
            index_v2 = index_v2[:, 1]*arange(1, max_particles)
            index_v2 = index_v2 * (index_v2 != 0)
            index_v3 = (valley[:, 1] == 3)
            index_v3 = index_v3[:, 1]*arange(1, max_particles)
            index_v3 = index_v3 * (index_v3 != 0)
            index_v4 = (valley[:, 1] == 4)
            index_v4 = index_v4[:, 1]*arange(1, max_particles)

...
def pn_init_v2(max_particles=None, dope_type=None, dx=None, dy=None, Ltot=None, nx1=None, ny1=None, cdop=None, ppc=None,
    ...
    p_total = sum(psi)
    psi = psi*arange(1, max_particles)
    psi = psi * (psi != 0)
    # --------------------Added 11/1------------------------------------
    ...
    n_total = sum(pdi)
    pdi = pdi * arange(1, max_particles)
    pdi = pdi * (pdi != 0)
    # --------------------Added 11/1------------------------------------
    n_icpg = zeros(ny1, 1)
    for i in range(1, length(pdi)+1):
        index = pdi[i]
        y = particles[index, 6]
        if y < dot(0.5, dy):
    ...
    return particles, valley, bg_charge, cpsp, N, p_icpg, n_icpg, xmax, ymax

...
for a=1:length(Vp_all)
    for b=1:length(Vn_all)
        ...
        input_data = cat(3,net_charge/1e23,border_cond,old_phi,phi);
        recorder = cat(4,recorder,input_data);
...
% 
        ...
        aa=Vp_all(a)*100;
        bb=Vn_all(b)*100;
        save(['./data/','Vp=',num2str(aa),'Vn=',num2str(bb),'dx=',num2str(dx*1e9),'nm.mat']);%bceause it can't save files like 0.1
        save(['./data/','Vp=',num2str(aa),'Vn=',num2str(bb),'dx=',num2str(dx*1e9),'nm_data.mat'],'recorder');%bceause it can't save files like 0.1
    end
end